...
print("START D")
import subprocess
osm_file =  "../ukraine-251022.osm.pbf"
args = ["osmium", "extract", "--bbox", ",".join([("{0:0.6f}").format(x) for x in bbox]), "--overwrite", "-o", "out.osm", osm_file]
subprocess.check_call(args)
args = ["osmium", "tags-filter", "out.osm", "w/highway!=path", "--overwrite", "-o", "out1.osm"]
subprocess.check_call(args)
features = osmnx.features.features_from_xml("out.osm", tags = {"building": True,})
network = osmnx.graph.graph_from_xml("out1.osm", simplify = True, retain_all = True)
print("END D")
